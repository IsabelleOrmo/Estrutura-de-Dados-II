/*
Resumo das funções:
nova_tabela_de_frequencias: Inicializa a tabela de frequências com valores nulos.

novo_no_de_arvore_binaria: Cria um novo nó de árvore binária (aloca memória, define filhos e dados).

inclua_byte: Conta a frequência de bytes e cria novos nós se necessário.

junte_nodos_no_inicio_do_vetor: Organiza os ponteiros para facilitar o próximo passo de construção da árvore.
*/
...
{
    *novo = (Ptr_de_no_de_arvore_binaria_de_busca)malloc(sizeof(Struct_do_no_de_arvore_binaria_de_busca));
